...
class TD3(object):
    ...
    #jangikim
    #def train(self, o_stats, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2,
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2,
        ...
        for it in range(iterations):
            ...
            '''
            t_x = o_stats.normalize(tf.convert_to_tensor(x))
            n_x = t_x.eval()
            t_y = o_stats.normalize(tf.convert_to_tensor(y))
            n_y = t_y.eval()
            '''

            state = torch.FloatTensor(x).to(device)
            #state = torch.FloatTensor(n_x).to(device)
            action = torch.FloatTensor(u).to(device)
            next_state = torch.FloatTensor(y).to(device)
            #next_state = torch.FloatTensor(n_y).to(device)
    ...
    def train2(self, x, y, u, r, d, it, discount=0.99, tau=0.005, policy_noise=0.2,
              noise_clip=0.5, policy_freq=2):
