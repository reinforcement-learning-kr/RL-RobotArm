...
from utilsTD3 import ReplayBuffer, Transition
...
class hrlTD3():
    ...
    def __init__(self,
        ...
        #self.target_meta_controller = TD3.TD3(state_dim, action_dim, max_action)
        self.controller = TD3.TD3(state_dim, action_dim, max_action)
        #self.target_controller = TD3.TD3(state_dim, action_dim, max_action)
    ...
    def select_goal(self, state, epilson):
        ...
        if sample > epilson:
            ...
            return self.meta_controller.actor(Variable(state, volatile=True)).data.max(1)[1].cpu()
        else:
    ...
    def select_action(self, joint_state_goal, epilson):
        ...
        if sample > epilson:
            ...
            return self.controller.actor(Variable(joint_state_goal, volatile=True)).data.max(1)[1].cpu()
        else:
    ...
    def update_meta_controller(self, episode_timesteps, args, gamma=1.0):
        if len(self.meta_replay_memory) < self.batch_size:
            return
        #jangikim
        self.meta_controller.train(self.meta_replay_memory, episode_timesteps, args.batch_size, args.discount, args.tau, args.policy_noise,
    ...
    def update_controller(self, episode_timesteps, args, gamma=1.0):
        if len(self.ctrl_replay_memory) < self.batch_size:
            return
        self.controller.train(self.ctrl_replay_memory, episode_timesteps, args.batch_size, args.discount, args.tau, args.policy_noise,
                     args.noise_clip, args.policy_freq)
        '''
